...
object TestGlobalLocationID3 {
    ...
    def main(args: Array[String]): Unit = {
        ...
        try {
            ...
            case e: Throwable => {
                println("ERROR when loading and evaluation")
                e.printStackTrace()
            }
        }
        ...
        outputstring = "%s\n%s".format(outputstring, "Evaluation of the full tree finished in " + (endTime - startTime)/1E9)
        
        startTime = System.nanoTime()
        
        val globalTreePruned = new ID3TreeBuilder()
        try {
            globalTreePruned.loadModelFromFile(model_path + "pruned")
            println("OK: Load tree from '%s' successfully".format(model_path + "pruned"))
            //println("tree:\n" + globalTree.treeModel.tree)
            ...
            val predictedValues = globalTreePruned.predict(dataForThisUser)
            val actualValues = dataForThisUser.map(x => {
                x.split(',').last
            })
        ...
        outputstring = "%s\n%s".format(outputstring, "Evaluation of the pruned tree finished in " + (endTime - startTime)/1E9)
        
        println(outputstring)
        //println("Evaluation of the full tree finished in " + (endTime - startTime)/1E9)
